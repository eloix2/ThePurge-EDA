 ...
 */
#define PLAYER_NAME BBstaBBlin62
...
struct PLAYER_NAME : public Player {
    ...
    */
    ...
    bool enemigo_cerca(int id_start, Pos p, int dist) {
        pair<Pos, int> move_to = search_for_enemies(id_start, p, true, dist);
    ...
    bool between_walls(Pos bar, Pos builder) {
        Dir d = Up;
        for (int i = 0; i < 4; ++i) if (bar+dirs[i] == builder) d = dirs[i];
        ...
        while (not cola.empty() and inside_max_distance) {
            ...
            if (id != -1) {
                ...
                if ( it == ids_aliados.end() and citizen(id).life > 0 and citizen(id).type == Builder ) {
                    ...
                    ++encontrados;
                }else if (it == ids_aliados.end() and citizen(id).life > 0) {
    ...
    virtual void play () {
        ...
        for (int id : b) { // Iterate over all builders
            ...
            if (is_day()) { //day
                ...
                if (max_num_barricades() > int(barricades(me()).size())) {
                    ...
                    if (move_to.first != p) {
                        ...
                        while (i < 4 and not built) {
                            Dir d = dirs[i];
                            if (p+d == move_to.first and cell(move_to.first).b_owner == me() and not built) {
                                build(id, d);
                                built = true;
                            }else if (p+d == move_to.first and cell(move_to.first).b_owner != me()){
                                move(id, d);
                                built = true;
                            }
                            ++i;
                ...
                bool temp = enemigo_cerca(id, p, 2);
                cerr << p << ' ' << temp << endl;
