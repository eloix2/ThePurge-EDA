 ...
 */
#define PLAYER_NAME BBstaBBlin24
    ...
    pair<Pos, int> search_for_enemies(Pos p, bool with_bazooka, int max_distance) {
        ...
        while (not cola.empty() and not encontrado and inside_max_distance) {
            ...
            if (id != -1 and with_bazooka and it == ids_aliados.end() and citizen(id).life > 0) {
                ...
                distancia = distancias[p.i][p.j];
                encontrado = true;
            }else if (id != -1 and it == ids_aliados.end() and citizen(id).life > 0 and citizen(id).type == Warrior and citizen(id).weapon != Bazooka) {
                objetivo = p;
                distancia = distancias[p.i][p.j];
                encontrado = true;
        ...
        for (int id : w) {
            ...
            if (has_bazooka) {
                if (need_health) move_to = search_for_items(p, Food, NoWeapon, true, 27);
                    ...
                    if (is_day())move_to2 = search_for_items(p, NoBonus, Bazooka, true, 7);
                ...
                if (need_health) move_to = search_for_items(p, Food, NoWeapon, false, 30);
                    ...
                    if (has_gun) {
                        move_to = search_for_items(p, NoBonus, Bazooka, false, 18);

                        if (move_to.first == p) move_to = search_for_items(p, NoBonus, NoWeapon, true, 25);
