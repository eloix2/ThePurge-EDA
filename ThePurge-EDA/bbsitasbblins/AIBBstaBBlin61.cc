 ...
 */
#define PLAYER_NAME BBstaBBlin61
...
struct PLAYER_NAME : public Player {
    ...
    */
    ...
    bool build_random_barricade(int id, Pos p) {
        int i = 0;
        bool built = false;
        ...
        while (not cola.empty() and  not(encontrado and not barricada_encontrada) and inside_max_distance) {
            ...
            if ((Weapon == NoWeapon and Bonus != NoBonus) or (Weapon != NoWeapon and Bonus == NoBonus)) {
                if ( (Weapon == NoWeapon) and (cell(p).bonus == Bonus) and distancias[p.i][p.j] < distancia) {
                    ...
                    encontrado = true;
                }else if ( (Bonus == NoBonus) and (cell(p).weapon == Weapon or cell(p).weapon == Bazooka) and distancias[p.i][p.j] < distancia ) {
                    objetivo = p;
                    distancia = distancias[p.i][p.j];
                    encontrado = true;
                }
            }else {
                if ((cell(p).weapon != NoWeapon or cell(p).bonus != NoBonus) and distancias[p.i][p.j] < distancia) {
                    objetivo = p;
                    distancia = distancias[p.i][p.j];
                    encontrado = true;
                }
    ...
    pair<Pos, int> search_for_enemies(int id_start, Pos p, bool with_bazooka, int max_distance) {
        set<int> ids_aliados = get_ids_aliados();
    ...
    pair<Pos, int> search_for_barricades(Pos p, bool _build, int max_distance) {
        ...
        while (not cola.empty() and not encontrado and inside_max_distance) {
            ...
            if (_build and cell(p).b_owner == me() and cell(p).resistance < barricade_max_resistance()/2 and p != p_backup) {
                objetivo = p;
                distancia = distancias[p.i][p.j];
                encontrado = true;
    ...
    virtual void play () {
            ...
            if (is_day()) { //day
                ...
                if (max_num_barricades() > int(barricades(me()).size())) {
                    ...
                    if (move_to.first != p) go_for_it(id, p, move_to.first);
                        ...
                        if (not build_random_barricade(id, p)) {
                    ...
                    if (move_to.first != p) {
                        ...
                        move_to = search_for_items(id, p, NoBonus, NoWeapon, true, 10);
                        if (move_to.first != p) {
                            go_for_it(id, p, move_to.first);
                        }
                    }
                    ...
                    move_to = search_for_items(id, p, NoBonus, Bazooka, true, 6); //  a por una pistola quiza?!
                    ...
                    if (move_to.first != p) {
                        ...
                        if(need_health) {
                            ...
                            if (move_to.first != p) {
                                ...
                                move_to = search_for_items(id, p, Money, NoWeapon, true, 20);
                                if (move_to.first != p) go_for_it(id, p, move_to.first);
                            }
                        }else{
                ...
                move_to = search_for_items(id, p, NoBonus, Gun, false, 4); //  a por una pistola quiza?!

                if (move_to.first == p) {
                    need_health = (builder_ini_life() - citizen(id).life > food_incr_life()*4); //si hubiera comida cerca, ir a por ella?!
                    if(need_health) {
                        move_to = search_for_items(id, p, Food, NoWeapon, false, 8);
                        if (move_to.first == p){
                            int id;
                ...
                if (move_to.first == p) move_to = search_for_items(id, p, NoBonus, NoWeapon, false, 50);
                if (move_to.first != p) go_for_it(id, p, move_to.first);
            }
        ...
        for (int id : w) {
            ...
            bool need_health;
            need_health = citizen(id).life < warrior_ini_life()*4/8;
            //if (has_bazooka) need_health = citizen(id).life < warrior_ini_life()*6/8;
            //else need_health = citizen(id).life < warrior_ini_life()/2;

             if (has_bazooka) {
                if (need_health) move_to = search_for_items(id, p, Food, NoWeapon, true, 27);
                else {
                    move_to = search_for_enemies(id, p, false, 14);
                    ...
                    if (move_to2.first != p) move_to = move_to2; //quiza no sirva de nada
                }
            }else {
                if (need_health) move_to = search_for_items(id, p, Food, NoWeapon, false, 30);
                else {
                    if (has_gun) {
                    ...
                    }else move_to = search_for_items(id, p, NoBonus, Bazooka, false, 25);
                }
            }
            if (move_to.first == p) move_to = search_for_items(id, p, NoBonus, NoWeapon, true, 60);
            if (move_to.first != p) go_for_it(id, p, move_to.first);
        }
    }
};
