 ...
 */
#define PLAYER_NAME BBsitaBBlin6
...
struct PLAYER_NAME : public Player
{
    ...
    bool between_walls(Pos bar, Pos builder)
    {
        Dir d = Up;
        for (int i = 0; i < 4; ++i)
            if (pos_ok(bar + dirs[i]) and bar + dirs[i] == builder)
                d = dirs[i];
    ...
    bool check_barricades()
    {
        ...
        {
            if (cell(bar).resistance == barricade_max_resistance())
                check = true;
            else
                return false;
        }
    ...
    bool celda_proxima(Pos p, const vector<vector<int>> &distancias)
    {
        for (Dir d : dirs)
        {
            if (pos_ok(p + d) and distancias[(p + d).i][(p + d).j] == 0)
                return true;
        }
        return false;
    }
    ...
    int get_destroy_bar(int id, Pos p)
    {
        ...
        if (citizen(id).weapon == NoWeapon)
        {
            if (cell(p).resistance % builder_strength_demolish() == 0)
            {
                ...
                res = (cell(p).resistance / builder_strength_demolish()) + 1;
        }
        else if (citizen(id).weapon == Hammer)
        {
            if (cell(p).resistance % hammer_strength_demolish() == 0)
            {
                ...
                res = (cell(p).resistance / hammer_strength_demolish()) + 1;
        }
        else if (citizen(id).weapon == Gun)
        {
            if (cell(p).resistance % gun_strength_demolish() == 0)
            {
                ...
                res = (cell(p).resistance / gun_strength_demolish()) + 1;
        }
        else
        {
            if (cell(p).resistance % bazooka_strength_demolish() == 0)
            {
                ...
                res = (cell(p).resistance / bazooka_strength_demolish()) + 1;
        }
        return res;
    }
    ...
    pair<Pos, int> search_for_items(int id_start, Pos p, BonusType Bonus, WeaponType Weapon, const bool _could_combat, int max_distance)
    {
        ...
        while (not cola.empty() and not(encontrado and not barricada_encontrada) and inside_max_distance)
        {
            ...
            if ((Weapon == NoWeapon and Bonus != NoBonus) or (Weapon != NoWeapon and Bonus == NoBonus))
            {
                ...
                else if ((Bonus == NoBonus) and (cell(p).weapon == Weapon or cell(p).weapon == Bazooka) and distancias[p.i][p.j] < distancia and cell(p).weapon == strongestWeapon(cell(objetivo).weapon, cell(p).weapon))
                { // aquiiiii
                    objetivo = p;
                    distancia = distancias[p.i][p.j];
                    encontrado = true;
    ...
    pair<Pos, int> search_for_enemies(int id_start, Pos p, bool with_bazooka, int max_distance, bool radar)
    {
        ...
        if (encontrados > 0 or (with_bazooka and bazooka_encontrados > 0))
        {
            ...
            {
                if (not war_better.empty())
                    vistima1 = *war_better.begin();
                else if (not war.empty())
                    vistima1 = *war.begin();
                else if (not bui.empty())
                    vistima1 = *bui.begin();
            }
            else
            {
                if (not bui.empty() and not war.empty())
                {
                    ...
                    if (vistima2.first < 5 or vistima2.first < (vistima1.first * 3) / 4)
                        vistima1 = vistima2; // cambiar el 3/4 por 2 si no mejora mucho.
                }
                else if (not war.empty())
                    vistima1 = *war.begin();
                else if (not bui.empty())
    ...
    virtual void play()
    {
        ...
        { // Iterate over all builders
            ...
            if (locks_war.find(id) == locks_war.end())
            { // logica para que vaya a por la vida entera si baja de cierto valor
            ...
            { // day
                ...
                else if (not half_resistance_all)
                {
                    ...
                    { // si no tiene ninguna barricada cerca
                        ...
                        if (move_to.first == p)
                        {
                            ...
                            if (move_to.first == p)
                                move_to = search_for_items(id, p, NoBonus, NoWeapon, false, 15);
                            if (move_to.first == p)
                                move_to = search_for_items(id, p, NoBonus, NoWeapon, false, 15);
                        }

                        if (move_to.first == p)
                            move_to = search_for_items(id, p, NoBonus, NoWeapon, false, 20);
                        if (move_to.first != p)
                            go_for_it(id, p, move_to.first);
                    }
                }
                    ...
                    if (move_to.first == p)
                    {
                        ...
                        */
                        ...
                        if (need_health)
                        {
                            ...
                            if (move_to.first == p)
                                move_to = search_for_items(id, p, Money, NoWeapon, false, 15);
                        }
                        else
                        {
                ...
                if (move_to.first == p)
                {
                    ...
                    */
                    if (need_health)
                    {
                        ...
                        if (move_to.first == p)
                            move_to = search_for_barricades(id, p, false, 30);
                    }
                    else
                    {
        ...
        for (int id : w)
        {
            ...
            else if (citizen(id).life == warrior_ini_life())
                locks_war.erase(id);
            ...
            if (has_bazooka)
            {
                ...
                else
                {
                    ...
                    if (is_day())
                        move_to2 = search_for_items(id, p, NoBonus, Bazooka, true, 10);
                    else
                        move_to2 = search_for_items(id, p, NoBonus, Bazooka, true, 3);
            ...
            else
            {
                ...
                else
                {
                    if (has_gun)
                    {
                        ...
                        move_to = search_for_items(id, p, NoBonus, Gun, false, 25);
                }
            }

            if (move_to.first == p)
                move_to = search_for_items(id, p, NoBonus, NoWeapon, false, 60);
            go_for_it(id, p, move_to.first);
        }
    }
};
