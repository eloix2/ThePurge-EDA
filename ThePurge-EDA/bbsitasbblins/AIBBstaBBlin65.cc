...
#define PLAYER_NAME BBstaBBlin65
...
struct PLAYER_NAME : public Player {
    ...
    set<int> locks_war;
    set<int> locks_bui;
    vector< vector<int> > proximidad_builders;
    vector< vector<int> > proximidad_guns;
    vector< vector<int> > proximidad_bazooka;
    ...
    void enemigo_cerca() {
        ...
        for (int h = 2; h < 5; ++h) {
            for (int i = 0; i < board_rows(); ++i) {
                for (int j = 0; j < board_cols(); ++j) {
                    Pos p = {i,j};
                    if (h == 2) {
                        move_to = search_for_enemies(-h, p, true, 3, true);
                        proximidad_builders[i][j] = move_to.first != p;
                    }else if (h == 3) {
                        move_to = search_for_enemies(-h, p, true, 3, true);
                        proximidad_guns[i][j] = move_to.first != p;
                    }else if (h == 4) {
                        move_to = search_for_enemies(-h, p, true, 2, true);
                        proximidad_bazooka[i][j] = move_to.first != p;
                    }
                }
            }
        }
        for (int i = 0; i < board_rows(); ++i) {
            for (int j = 0; j < board_cols(); ++j) {
                Pos p = {i,j};
                if (cell(p).id != -1 and citizen(cell(p).id).player != me()) {
                    proximidad_builders[p.i][p.j] = -1;
                    proximidad_guns[p.i][p.j] = -1;
                    proximidad_bazooka[p.i][p.j] = -1;
                }
            }
        }
    }
    ...
    int get_destroy_bar(WeaponType weapon, Pos p) {
        int res = 0;
        if (weapon == NoWeapon) {
        ...
        while (not cola.empty() and  not(encontrado and not barricada_encontrada) and inside_max_distance) {
            ...
            for (Dir d : dirs) {
                if (pos_ok(p+d) and cell(p+d).type == Street) {
                    ...
                    bool cond;
                    if (arma == NoWeapon or arma == Hammer) cond = proximidad_builders[(p+d).i][(p+d).j];
                    else if (arma == Gun) cond = proximidad_guns[(p+d).i][(p+d).j];
                    else if (arma == Bazooka) cond = proximidad_bazooka[(p+d).i][(p+d).j];

                    if (id == -1 and (cell(p+d).resistance == -1 or cell(p+d).b_owner == me())) {

                        if (is_day() or (is_night() and not cond)) {
                            //cerr << id_start << endl;
                        ...
                        if (not cond) {
                            ...
                            int steps_to_destroy = get_destroy_bar(arma, p+d);
    ...
    pair<Pos, int> search_for_enemies(int id_start, Pos p, bool with_bazooka, int max_distance, bool radar) {
        ...
        WeaponType arma;
        if (id_start == -2) arma = NoWeapon;
        else if (id_start == -3) arma = Gun;
        else if (id_start == -4) arma = Bazooka;
        else arma = citizen(id_start).weapon;
    ...
    pair<Pos, int> nearest_friend_warrior(int id_start, Pos p, int& r_id, int max_distance) {
     ...
     */
    virtual void play () {
        proximidad_builders = vector<vector<int>>(board_rows(), vector<int> (board_cols(), false));
        proximidad_guns = vector<vector<int>>(board_rows(), vector<int> (board_cols(), false));
        proximidad_bazooka = vector<vector<int>>(board_rows(), vector<int> (board_cols(), false));
